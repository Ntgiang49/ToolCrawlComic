...
def enumerate_valid(session: requests.Session, comic_url: str, max_n: int) -> list[int]:
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        results = executor.map(_check_ch, range(1, max_n + 1))
        for res in results:
            if res is not None:
                valid.append(res)
...
def main() -> None:
    ...
    for url, item in library.items():
        ...
        if not nums:
            ...
            continue
        max_n = max(nums)
        # ponytail: recent-20 list shifts over time → probe 300 past visible max,
        # cap at 3000. Ceiling: comic >3000 chapters needs a bigger cap.
